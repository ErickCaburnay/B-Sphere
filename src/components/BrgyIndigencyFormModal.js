...
export default function BrgyIndigencyFormModal({ isOpen, onClose }) {
  ...
  const [purpose, setPurpose] = useState(""); // Specific to Indigency, e.g., "for medical assistance", "for schooling"
  ...
  const handlePrint = () => {
    alert("Printing Barangay Indigency Form...");
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center z-50" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
      <div className="bg-white p-6 rounded-lg shadow-xl relative w-full max-w-lg mx-4">
        ...
        <h2 className="text-2xl font-bold mb-6 text-gray-800">Barangay Indigency Form</h2>

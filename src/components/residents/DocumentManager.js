...
export function DocumentManager({ residentId, onUpload, onDelete }) {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">Resident Documents {residentId ? `for Resident ID: ${residentId}` : ''}</h3>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        {documents.length > 0 ? (
          ...
          <p className="text-center text-gray-500 py-8">No documents uploaded for this resident yet.</p>
        )}
      </div>
    </div>
  );
